...
import { Sprout, Loader2, Mail, Lock, User, Brain, Wifi, BarChart3 } from "lucide-react";
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      toast({
        ...
        description: "Welcome to AgriSense AI.",
      });
      navigate("/");
    } catch (error: any) {
      toast({
      ...
      <div className="hidden md:flex flex-1 flex-col justify-center items-start p-12 lg:p-24 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-md">
          ...
          <p className="text-xl text-muted-foreground mb-12">
            Create an account to start predicting crop yields and monitoring your farms with AI.
          </p>
                ...
                render={({ field }: any) => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-2 text-sm font-medium">
                      <User className="h-4 w-4 text-purple-400" /> Full Name
                    </FormLabel>
                    <FormControl>
                      <Input placeholder="John Doe" {...field} className="h-12 rounded-xl bg-black/[0.04] dark:bg-white/[0.04] border-black/[0.1] dark:border-white/[0.08] focus:border-emerald-500/50 transition-colors" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
